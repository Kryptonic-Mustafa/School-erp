...
def step29_deploy():
  ...
  grades        Grade[]
    ...
    if "classId" in schema_content and "model Student" in schema_content:
        schema_content = schema_content.replace(old_student, new_student)
        with open(schema_path, "w", encoding="utf-8") as f:
            f.write(schema_content)
        run_command("npx prisma generate", cwd=project_dir)
        run_command("npx prisma db push", cwd=project_dir)
    else:
        print("  [-] Schema already updated to Many-to-Many.")

    print("\n>>> 2. REWRITING STUDENT API KERNEL...")
    api_students = """
...
"""
    create_file(os.path.join(project_dir, "src/app/api/students/route.ts"), api_students)

    print("\n>>> 3. REWRITING STUDENTS UI (MULTI-SELECT BADGE SYSTEM)...")
    students_ui = """
'use client';
...
"""
    ...
    print("\n>>> 4. PATCHING ATTENDANCE ROUTES FOR MULTI-CLASS COMPATIBILITY...")
    api_attendance_path = os.path.join(project_dir, "src/app/api/attendance/route.ts")
    if os.path.exists(api_attendance_path):
        with open(api_attendance_path, "r", encoding="utf-8") as f:
            att_api = f.read()
        att_api = att_api.replace("include: {\n        class: true,", "include: {\n        classes: true,")
        create_file(api_attendance_path, att_api)

    ui_attendance_path = os.path.join(project_dir, "src/app/admin/attendance/page.tsx")
    if os.path.exists(ui_attendance_path):
        with open(ui_attendance_path, "r", encoding="utf-8") as f:
            att_ui = f.read()
        att_ui = att_ui.replace("student.class?.name", "student.classes?.map((c:any)=>c.name).join(', ')")
        create_file(ui_attendance_path, att_ui)

    print("\n====================================================")
    print(" [SUCCESS] MULTI-CLASS STUDENT ARCHITECTURE ONLINE. ")
    print("====================================================\n")
